...
This vignette demonstrates a **replication of the Majora’s Mask Dog Race** minigame mechanics in R. Our work is inspired by a Vidya James YouTube video [Why Majora's Mask's Blue Dog Took 25 Years to Win the Race](https://www.youtube.com/watch?v=Y1l6Xj4PLEk), which discusses how the race functions in-game, and the information available from the **original C source code** hosted on the Majora's Mask [zeldaret](https://github.com/zeldaret/mm) GitHub repository. The primary objective is to **estimate** each dog’s likelihood of winning by running a large number of **Monte Carlo** (random) simulations under the same rules used by the game.
...
4. **Observing the Blue Dog**  
   Many players believe the **Blue dog never wins**, because while the Blue dog starts with a high speed for the first quarter, it can perform poorly afterwards. We aim to verify this empirically by simulating many races and examining the results.
...
- **Custom RNG**: Majora’s Mask uses a Linear Congruential Generator (LCG) for random floats, i.e., it updates a 32-bit integer state and converts it to a float in \([-0.5, +0.5]\).  
...
We start by defining the dog speeds (`sBaseSpeeds`), the 14 dog colours (`dog_colours`), and the condition distribution (4 good, 5 neutral, 5 bad). We also store a small table, shown below, for reference:
...
5. Repeats for \(\text{n\_sims}\) runs, so we can **count** how many times each dog finishes first.
...
When we select the Gold dog, we see in Figure 1 below that “gold_1” has an **empirical winning rate** of around 28%, while “blue_1” may have 0%, indicating that across 100,000 trials, Blue did not win once in this scenario.
...
Figure 2 shows the **mean finishing position** of each dog for the gold-selected scenario. Lower means a better average finish (i.e., they place nearer 1st more often). Gold naturally does well, with a mean finish near 5.6, while Blue might be near 9.4, reflecting its struggle to maintain speed in later segments.
...
Figure 3 displays how often each dog finishes at each rank (1..14). Notice that many dogs have multi-modal distributions, reflecting the interplay of condition (good, neutral, bad) and random fluctuations.
...
Figure 6 shows the rank density plots again, still with multi-modal patterns for each dog.
...
Figure 8 plots the **difference** in each dog’s winning rate between “Gold selected” and “Blue selected” simulations:
